...
func (s *ChatService) SendMessage(ctx context.Context, msg chatdomain.ChatMessage) error {
	if !msg.JobID.IsZero() {
		job, err := s.JobRepo.GetJobByID(msg.JobID)
		if err != nil {
			return fmt.Errorf("no se encontró el trabajo: %v", err)
		}
		// Verificar que el trabajo no esté completado.
		if job.Status == jobdomain.JobStatusCompleted {
			return fmt.Errorf("no se puede chatear: el trabajo está completado")
		}
		// Validar que los participantes sean el empleador y el trabajador asignado.
		if job.AssignedTo == nil {
			return fmt.Errorf("el trabajo no tiene asignado un trabajador")
		}
		...
		if !validParticipants {
			return fmt.Errorf("el chat solo está permitido entre el empleador y el trabajador asignado")
		}
	}
